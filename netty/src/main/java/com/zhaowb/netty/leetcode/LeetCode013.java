...
public class LeetCode013 {
    ...
    public static void main(String[] args) {
        ...
        int t = leetCode013.romanToInt("MCMXCIV");
    ...
    public int romanToInt(String s) {
        ...
        for (int i = charArr.length - 1, j = i; i >= 0; i = j - 1, j = i) {
            c = charArr[i];
            while (j - 1 >= 0 && map.get(c) > map.get(charArr[j - 1])) {
                j--;
            }
            sum += (map.get(c) - romanToInt(s.substring(j, i)));
        }
        return sum;
    }
}